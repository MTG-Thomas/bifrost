...
If the user's request authorizes the action, call bifrost_execute_tool with a
stable caller-generated operation_id. Reuse that operation_id when retrying a
request whose response may have been lost. Modern MCP clients may request an
MCP Task for durable workflow, delegation, or platform-job operations; Bifrost
maps that task to its canonical lifecycle. bifrost_get_execution can inspect
an exact execution ID and page large results. If the request does not authorize
execution, offer the action instead of executing it.
...
async def bifrost_execute_tool(
    ...
    result = data["result"]
    if data.get("source") == "workflow" and isinstance(result, dict):
...
TOOLS = [
    ...
    (
        ...
        "when retrying a request whose response may have been lost. Durable "
        "operations support the MCP Tasks protocol.",
    ),
    (
