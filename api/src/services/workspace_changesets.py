"""Transactional orchestration for remote workspace changes.
...
from src.repositories.workspace_changesets import WorkspaceChangesetRepository
...
class WorkspaceChangesetService:
    ...
    def normalize_scope(scope: str) -> str:
        ...
        if not value or value in {".", ".."} or ".." in PurePosixPath(value).parts:
            raise ChangesetInvalid("scope must be a non-empty workspace-relative prefix")
        return value
    ...
    def normalize_path(cls, path: str, scope: str) -> str:
        ...
        if not value or ".." in PurePosixPath(value).parts:
            raise ChangesetInvalid("path must be workspace-relative and cannot contain '..'")
        if value != scope and not value.startswith(f"{scope}/"):
            raise ChangesetInvalid(f"path {value!r} is outside changeset scope {scope!r}")
        return value
    ...
    async def get(self, changeset_id: UUID) -> WorkspaceChangesetResponse:
        return self._response(await self._required(changeset_id))

    async def stage(self, changeset_id: UUID, request: WorkspaceFileMutationRequest) -> WorkspaceChangesetResponse:
        ...
        if request.expected_hash is not None and request.expected_hash != before_hash:
            raise ChangesetConflict("file_revision_mismatch", path=path, expected_hash=request.expected_hash, current_hash=before_hash)
        content = None
        after_hash = None
        ...
        mutation = {"path": path, "operation": request.operation, "content_base64": content, "before_hash": before_hash, "after_hash": after_hash, "force_deactivation": request.force_deactivation}
    ...
    async def diff(self, changeset_id: UUID) -> WorkspaceChangesetDiffResponse:
        ...
        return WorkspaceChangesetDiffResponse(changeset_id=row.id, files=result)

    async def validate(self, changeset_id: UUID) -> WorkspaceValidationResponse:
        row = await self._required(changeset_id, for_update=True)
        self._ensure_active(row)
        ...
        for item in row.mutations:
            ...
            if item["operation"] == "write":
                ...
                try:
                    ...
                    diagnostics.append({"path": path, "severity": "error", "source": "syntax", "message": str(exc), "line": getattr(exc, "lineno", None), "column": getattr(exc, "offset", None)})
                    continue
                for node in ast.walk(tree):
                    ...
                    for decorator in node.decorator_list:
                        ...
                        if parsed and parsed[0] in {"workflow", "tool", "data_provider"}:
                            kind, kwargs = parsed
                            names.add(node.name)
                            decorator_info[node.name] = (kind, kwargs.get("name") or node.name)
            if not item.get("force_deactivation"):
                found, _ = await deactivation.detect_pending_deactivations(path=path, new_function_names=names, new_decorator_info=decorator_info)
        ...
        result = WorkspaceValidationResponse(valid=valid, diagnostics=diagnostics, pending_deactivations=pending, validated_revision=current_revision)
        row.validation = result.model_dump(mode="json")
        row.status = "validated" if valid else "staged"
        await self.db.flush()
        return result

    async def activate(self, changeset_id: UUID, request: WorkspaceActivateRequest, updated_by: str) -> WorkspaceChangesetResponse:
        # PostgreSQL transaction-scoped serialization shared by all API replicas.
        await self.db.execute(text("SELECT pg_advisory_xact_lock(hashtext('bifrost:workspace-changesets'))"))
        row = await self._required(changeset_id, for_update=True)
        if row.status != "validated" or not row.validation or not row.validation.get("valid"):
            raise ChangesetInvalid("changeset must pass validation immediately before activation")
        current_revision, current_files = await self._snapshot(row.scope)
        conflicting = [item["path"] for item in row.mutations if current_files.get(item["path"]) != item.get("before_hash")]
        if conflicting:
            row.status = "conflicted"
            await self.db.flush()
            await self.db.commit()
            raise ChangesetConflict("revision_mismatch", base_revision=row.base_revision, current_revision=current_revision, conflicting_paths=conflicting)
        row.status = "activating"
        originals = {item["path"]: await self._read_optional(item["path"]) for item in row.mutations}
        ...
        try:
            for item in row.mutations:
                if item["operation"] == "delete":
                    ...
                    result = await storage.write_file(item["path"], base64.b64decode(item["content_base64"]), updated_by=updated_by, force_deactivation=bool(item.get("force_deactivation")))
                    if result.pending_deactivations:
                        raise ChangesetInvalid(f"deactivation preflight changed for {item['path']}")
    ...
    def _ensure_active(self, row: WorkspaceChangeset) -> None:
        if row.status not in self.ACTIVE:
            raise ChangesetInvalid(f"changeset in {row.status!r} state cannot be modified")
    ...
    @staticmethod
    def _unified_diff(path: str, before: bytes | None, after: bytes | None) -> str | None:
        try:
    ...
    @staticmethod
    def _response(row: WorkspaceChangeset) -> WorkspaceChangesetResponse:
        return WorkspaceChangesetResponse(id=row.id, scope=row.scope, base_revision=row.base_revision, status=cast(ChangesetStatus, row.status), title=row.title, worker_id=row.worker_id, mutations=[WorkspaceMutation.model_validate(item) for item in row.mutations], validation=row.validation, activated_revision=row.activated_revision, commit_sha=row.commit_sha, error=row.error, created_at=row.created_at, updated_at=row.updated_at)
