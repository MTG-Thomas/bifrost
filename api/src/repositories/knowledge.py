...
class KnowledgeRepository(OrgScopedRepository[KnowledgeStore]):
    ...
    async def search(
        ...
        vector_stmt = self._apply_search_scope(vector_stmt, target_org_id, fallback)
        ...
        if not lexical_query:
            ...
            lexical_stmt = self._apply_search_scope(
                lexical_stmt, target_org_id, fallback
            )
        ...
        return self._search_rows_to_documents(
            ranked_rows, min_score, group_by_key, limit
        )
