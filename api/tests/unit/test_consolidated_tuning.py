...
def _org_principal(user, organization_id) -> UserPrincipal:  # type: ignore[no-untyped-def]
    return UserPrincipal(
        user_id=user.id,
        email=user.email,
        name=user.name or "",
    ...
    try:
        with patch.object(
            ...
            proposal = await propose_consolidated_tuning(
                seed_agent.id,
                db_session,
                _org_principal(seed_user, caller_org_id),
            )
...
async def test_apply_only_clears_caller_org_flagged_runs_for_global_agent(
    ...
    try:
        applied = await apply_consolidated_tuning(
            ...
            user_id=seed_user.id,
            db=db_session,
            user=_org_principal(seed_user, caller_org_id),
        )
        await db_session.commit()
