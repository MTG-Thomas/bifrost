...
def test_pull_request_head_ref_detects_dependency_changes(tmp_path: Path):
    repo = tmp_path / "repo"
    ...
    (repo / "requirements.lock").write_text("pytest==1\n")
    ...
    (repo / "requirements.lock").write_text("pytest==2\n")
