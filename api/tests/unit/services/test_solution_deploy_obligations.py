...
def test_vendored_python_is_bound_as_derived_candidate_content() -> None:
    entries = [
        ("bifrost.solution.yaml", b"slug: example\nname: Example\n"),
        ("functions/main.py", b"value = 1\n"),
    ]
    ...
    assert valid is True
    assert reason is None
    assert evidence["derived_paths"] == [
        "solutions/example/shared/vendor.py"
    ]
...
async def test_source_declaration_returns_exact_solution_evidence() -> None:
    ...
    assert response.solution_deploy_obligations == [
