...
3. **Run the packager** (path is relative to this skill directory):
   ```bash
   python3 "${CLAUDE_PLUGIN_ROOT}/skills/bifrost-copilot-cowork-package/pack.py" "<agent name or id>" --out ./out
   ```
...
**`DynamicClientRegistration` is in the broader manifest schema but Cowork's MCP runtime does NOT support it today.** Even though Bifrost MCP servers natively support RFC 7591 DCR (which is why Codex.ai's "add MCP" flow just works), don't use it for Cowork — the upload will be rejected with `Invalid encoded OAuthConfigurationId`.
...
- The official Microsoft conversion script is PowerShell and converts Codex-plugin directories. This skill does NOT depend on it — it talks to Bifrost directly via the CLI and builds the package in Python.
...
```
$ python3 "${CLAUDE_PLUGIN_ROOT}/skills/bifrost-copilot-cowork-package/pack.py" "Cyber Questionnaire Assistant" --out /tmp
{
