...
export function useWorkflowMutation<T = unknown>(
	...
	const execute = useCallback(
		async (params?: Record<string, unknown>): Promise<T> => {
			...
			// Check once immediately after subscribing so query hooks don't get stuck
			// with null data when the completion event was missed.
			try {
				const execution = await getExecution(execId);
				if (
				...
				// Ignore immediate status fetch failures; WebSocket path still handles normal execution flow.
			}
