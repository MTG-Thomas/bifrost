...
- A fork head may merge only while it already contains current `main`; the queue
  cannot write to fork branches and removes stale fork heads fail closed.
...
CodeQL language analyses, `CodeRabbit`, and `SonarCloud`. CI cancels superseded
...
GitHub's Ed25519 host key, writes the secret into a mode-0600 temporary directory,
and deletes it when the step exits. The key is used only to update a queued
same-repository branch; GitHub's scoped workflow token performs API reads,
labels, comments, and the protected merge.
