...
if (!["localhost", "127.0.0.1"].includes(BASE_URL.hostname)) {
  throw new Error(
    `files gallery demo setup only uploads fixtures to localhost, got ${BASE_URL.origin}`,
  );
}
...
async function putPolicy(prefix, rules, location = "workspace", scope) {
  ...
  const r = await api(
    "PUT",
    `/api/files/policies/${encodeURIComponent(prefix)}?${qs}`,
    { policies: { policies: rules } },
  );
  if (!r.ok)
    throw new Error(
      `putPolicy ${location}/${prefix} scope=${scope}: ${r.status} ${JSON.stringify(r.json)}`,
    );
...
async function ensureUser(email, orgId) {
  ...
  if (c.ok) {
    log("created user", email, "in org", orgId);
    return c.json.id;
  }
  if (c.status === 409) {
...
if (prior) {
  ...
  if (created.ok) {
    ...
    appId = again.id;
    log("recovered existing app", appId);
  }
}
...
await putPolicy(
  ...
  [
    {
      name: "everyone-read",
      description: "Anyone may browse.",
      actions: ["read", "list"],
      when: null,
    },
    {
      name: "authed-write",
      description: "Logged-in users upload/delete.",
...
await putPolicy(
  ...
  [
    {
      name: "orgb-readonly",
      description: "Org B is read-only here.",
      actions: ["read", "list"],
      when: null,
    },
  ],
...
for (const [rel, file] of [
  ...
  const w = await api("POST", "/api/files/write", {
    path: rel,
    content: enc(readFileSync(file, "utf-8")),
    mode: "cloud",
    location: "workspace",
    binary: true,
  });
...
const pub = await api("POST", `/api/applications/${appId}/publish`, {
...
await seed(
  "global-readme.txt",
  txt("This file lives in shared/global/gallery — the cross-org pool.\n"),
  "text/plain",
  "global",
);
...
await seed(
  "orgA-note.txt",
  txt("Only Org A users see this (shared/<orgA>/gallery).\n"),
  "text/plain",
  orgA,
);
// org B pool
...
log(
  "Admin:          dev@gobifrost.com / password   (sees its OWN org's gallery by default; global via scope)",
);
log(
  "Alice (Org A):  alice@gallery.example.com / password  (org A gallery, can upload)",
);
log(
  "Bob (Org B):    bob@gallery.example.com / password    (org B gallery, READ-ONLY via override)",
);
